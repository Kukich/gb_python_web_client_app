...
class ServerSocket(ClientServer):
    ...
    def start_server(self,transport):
        ...
        while True:
            ...
            try:
                self.log.info("Сервер получил сообщение от клиента")
                message_from_cient = self.get_message(client)
                print(message_from_cient)
                # {'action': 'presence', 'time': 1573760672.167031, 'user': {'account_name': 'Guest'}}
                response = self.process_client_message(message_from_cient)
                self.log.info("Сервер отвечает клиенту")
                self.log.info(pformat(response))
                self.send_message(client, response)
                client.close()
            except (ValueError, json.JSONDecodeError):
                self.log.error('Принято некорретное сообщение от клиента.')
                client.close()